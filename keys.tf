...
resource "null_resource" "write_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      until rsync -e "ssh -i ${var.ssh_private_key_file} -o ConnectTimeout=15 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no" --rsync-path="sudo rsync" ${var.servers[0].system_user}@${local.external_ip}:/etc/rancher/rke2/rke2.yaml rke2.yaml >/dev/null 2>&1; do echo Wait rke2.yaml generation && sleep 5; done \
